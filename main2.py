...
for i, row in tqdm(points[25000:].iterrows()):
    ...
    try:
        ...
        if i % 10000 == 0 and i != 0:
            ...
            geo_dataframe_temp.to_file(BASE / f"tables/stratified_ALL_S2_points_wdate_filter_{i}_2.gpkg", driver="GPKG")
            geo_dataframe_temp.drop(columns=["geometry"]).to_csv(BASE / f"tables/stratified_ALL_S2_points_wdate_filter_{i}_2.csv",
                                                            index=False)
...
# Save as a GeoPackage (vector file) and CSV
geo_dataframe.to_file(BASE / "tables/stratified_ALL_S2_points_wdate_filter_2.gpkg", driver="GPKG")
geo_dataframe.drop(columns=["geometry"]).to_csv(BASE / "tables/stratified_ALL_S2_points_wdate_filter_2.csv", index=False)
